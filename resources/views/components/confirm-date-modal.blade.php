        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center md:items-center sm:flex sm:p-0">
            ...
            <div x-cloak x-show="true" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <div class="flex items-center justify-between space-x-4">
                    <h1 class="text-xl font-medium text-yellow-500 ">
                        {{ trans('generalTrans.Reschedule-title') }}
                    </h1>
                    <button wire:click.prevent="confirmEventChange" @click="modelOpen = false"
                ...
                <form wire:submit="submit" class="mt-5 rtl:text-start">
                    <h1>
                        {{ trans('generalTrans.Reschedule-description') }}
                    </h1>
                    <div class="flex mt-6 gap-3">
                        <button wire:click.prevent="changeEvent"
                            class="flex w-full items-center justify-center px-3 py-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-yellow-500 rounded-md dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:bg-yellow-700 hover:bg-yellow-600 focus:outline-none focus:bg-yellow-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
                        ...
                        <button @click.prevent="modelOpen = false" wire:click.prevent="confirmEventChange"
