    ...
    <div x-show="modelOpen" x-on:close-modal.window = "modelOpen = false" class="fixed inset-0 z-50 overflow-y-auto"
        ...
        <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
            ...
            <div x-cloak x-show="true" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <div x-data="{ modelOpen: false }" class="flex justify-end mt-6">
                    <a wire:click="OpenModalEvent" @click="modelOpen =!modelOpen"
                        class="flex items-center px-3 py-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-md dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50 cursor-pointer">
                        Select Students To Promote
                        <x-loading-indecator :loading_action="'StudentsPromotionList'"></x-loading-indecator>
                    </a>
                    <x-dashboard.promotions.modals.student-list
                        name="open-student-modal"></x-dashboard.promotions.modals.student-list>
                </div>
