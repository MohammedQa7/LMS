<div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
    <ul class="space-y-2 font-medium">
        ...
        @if (Auth::user()->hasRole('administrator') ||
            ...
            <li>
                <button type="button"
                    ...
                    <span
                        class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">{{ trans('sidebar-trans.Teachers') }}</span>
        ...
        @if (Auth::user()->hasRole('administrator') ||
            ...
            <li>
                <button type="button"
                    ...
                    <span
                        class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">{{ trans('sidebar-trans.Students') }}</span>
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 10 6">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="m1 1 4 4 4-4" />
        ...
        @endif
        {{-- LEVEL --}}
        @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Levels') || Auth::user()->can('Create Level'))
            <li class="mb-2">
                <button type="button"
                    class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
                ...
                <ul id="level-dropdown" class="hidden py-2 space-y-2">
                    ...
                    @endif
                    @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Levels'))
                        <li>
        ...
        @endif
        {{-- CLASS --}}
        @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Classes') || Auth::user()->can('Create Class'))
            <li>
                <button type="button"
                    class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
                ...
                <ul id="class-dropdown" class="hidden py-2 space-y-2">
                    ...
                    @endif
                    @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Classes'))
                        <li>
        ...
        {{-- SECTION --}}
        @if (Auth::user()->hasRole('administrator') ||
                Auth::user()->can('Manage Sections') ||
                Auth::user()->can('Create Section'))
            <li>
                <button type="button"
                    class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
        ...
        @endif
        {{-- ZOOM --}}
        @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Zoom Meeting'))
            <li>
        ...
        @endif
        {{-- ROLES --}}
        @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Roles'))
            <li>
                <a href="{{ route('roles.index') }}"
                    ...
                    <span class="ms-3"> {{ trans('sidebar-trans.Roles-Permissions') }}</span>
                </a>
            </li>
        @endif

        {{-- STUDENT PROMOTIONS --}}
        @if (Auth::user()->hasRole('administrator') || Auth::user()->can('Manage Student Promotions'))
            <li>
                <a href="{{ route('student-promotion.index') }}"
                    class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
